...
public class AiAnalysisController {
    ...
    public SseEmitter analysis(@RequestBody Map<String, String> body) { // 请求体为 JSON map
        String content = body.get("content"); // 获取分析内容
        ...
        executor.execute(() -> {
            try {
                aiService.streamAnalysis(content, chunk -> { // 调用 AI 服务的流式分析方法，chunk 为每个词块
                    try {
        ...
        emitter.onCompletion(() -> { // 注册完成回调
            aiService.close(); // 确保 AI 连接关闭
        });
